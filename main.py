...
def main(page: ft.Page):
    ...
    def cargar_tabla_asistencia():
        ...
        for al in curso_seleccionado.get("alumnos", []):
            ...
            fila = ft.Container(
                ...
                content=ft.Row(
                    [
                        ft.Text(f"{al['id']}. {al['nombre']}", weight=ft.FontWeight.BOLD, size=12, color="#5C3A21", expand=True),
                        radio_asistencia
                    ],
    ...
    def cargar_tabla_notas():
        ...
        for al in curso_seleccionado.get("alumnos", []):
            ...
            fila = ft.Container(
                ...
                content=ft.Row(
                    [
                        ft.Text(f"{al['id']}. {al['nombre']}", weight=ft.FontWeight.BOLD, size=11, color="#5C3A21", expand=True),
    ...
    def refrescar_lista_cursos():
                        ...
                        [
                            ft.Column(
                                [
                                    ...
                                    ft.Row([
                                        ft.Icon(ft.icons.PEOPLE, size=14, color="#8C5A32"),
                                        ft.Text(f"{len(c.get('alumnos', []))} alumnos digitalizados", size=11, color="#8C5A32"),
                                        ft.Text(" • [TUTOR]" if c.get('es_tutor') else "", size=11, color="green", weight=ft.FontWeight.BOLD),
                                    ])
                                ],
                                spacing=4,
                                expand=True,
        ...
        def abrir_detalle_curso(curso):
            ...
            # Asignación diferida para evitar errores si la variable no ha cargado
            if 'vista_principal' in locals() or 'vista_principal' in globals():
                vista_principal.content = vista_tabla_asistencia
            if 'barra_navegacion' in locals() or 'barra_navegacion' in globals():
                barra_navegacion.selected_index = 1
            page.update()

        # --- MÓDULO OCR Y ESCANEO ---
        ...
        lbl_estado_escaneo = ft.Text("📷 Presione para capturar la lista", size=11, color="#8C5A32",
        ...
        def tomar_foto_camara(e):
            ...
            visor_camara.content = ft.Column(
                [
                    ft.Icon(ft.icons.CHECK_CIRCLE, color="white", size=28),
        ...
        # Formulario modal de creación de cursos
        ...
        txt_asig_tutor = ft.TextField(label="Asignatura(s) adicional(es)", hint_text="Ej: Historia", border_radius=8,
                                      visible=False)
        ...
        def guardar_curso_accion(e):
            ...
            alumnos_a_guardar = alumnos_temporal_ocr if alumnos_temporal_ocr else [
                {"id": i, "nombre": f"Estudiante {i}", "asistencia": "Presente", "n1": "0.00", "n2": "0.00",
                 "n3": "0.00"}
                for i in range(1, 21)
            ]
        ...
        modal_crear_curso = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Cancelar",
                              on_click=lambda e: setattr(modal_crear_curso, "open", False) or page.update()),
                ft.ElevatedButton(
                    "💾 GUARDAR CURSO",
            ...
            content=ft.Column(
                [
                    ft.Row(
                        [
                            ft.Column([
                                ft.Text("EduSmart", size=20, weight=ft.FontWeight.BOLD, color="#5C3A21"),
                                ft.Text("Panel del Docente", size=12, color="#8C5A32")
                            ]),
                            ft.ElevatedButton(
                                "Salir",
                                style=ft.ButtonStyle(color="white", bgcolor="#A93226",
        ...
        login_view = ft.Container(
            ...
            content=ft.Column(
                [
                    ...
                    ft.Text("EduSmart", size=24, weight=ft.FontWeight.BOLD, color="#5C3A21"),
                    ft.Text("U.E. 17 de Septiembre De San Francisco de Milagro", size=12, color="#8C5A32"),
        ...
        def cambiar_pestana(e):
            ...
            if index == 0:
                ...
                if curso_seleccionado:
                    ...
                    page.snack_bar = ft.SnackBar(ft.Text("⚠️ Primero seleccione un curso de la lista"),
                                                 bgcolor="orange")
                    page.snack_bar.open = True
            elif index == 2:
                if curso_seleccionado:
        ...
        barra_navegacion = ft.NavigationBar(
            ...
            visible=False,
            destinations=[
                ft.NavigationDestination(icon=ft.icons.CLASS_OUTLINED, selected_icon=ft.icons.CLASS, label="Cursos"),
                ft.NavigationDestination(icon=ft.icons.HOW_TO_REG_OUTLINED, selected_icon=ft.icons.HOW_TO_REG,
                                         label="Asistencia"),
                ft.NavigationDestination(icon=ft.icons.GRADE_OUTLINED, selected_icon=ft.icons.GRADE, label="Notas")
            ]
        )
        ...
        app_bar = ft.AppBar(
            title=ft.Row(
                [
                    ...
                    ft.Text("EduSmart", weight=ft.FontWeight.BOLD, color="white", size=18)
                ],
                spacing=10
            ),
            bgcolor="#5C3A21",
        ...
        # Configuración de página e inicio
        page.appbar = app_bar
        page.navigation_bar = barra_navegacion
        page.add(splash_view)
        page.update()

        # Transición Splash Screen -> Login
        time.sleep(1.5)
        page.controls.clear()
        page.add(login_view)
        page.update()
